...
In this course we will try to predict from the isavuconazole simulation a trough concentration at SS lower than 2 mg/L
...
creation_event <- isavuco %>% filter(time == 192) %>% 
  mutate(event = if_else(out > 2, 1, 0), event = as.factor(event)) %>% 
  select(id, event) 
...
# proportion
creation_event %>% summarise(prop_event = mean(event))
```
...
best_penalty <- 
  tune_lm %>%
  select_by_one_std_err(-penalty, metric = "accuracy")
...
final_res_predictions %>% accuracy(event, .pred_class)
final_res_predictions %>% yardstick::sens(event, .pred_class) #, event_level="second", estimator = "binary")
final_res_predictions %>% yardstick::spec(event, .pred_class)
final_res_predictions %>% yardstick::precision(event, .pred_class)# precision = vpp
final_res_predictions %>% yardstick::recall(event, .pred_class)# precision = vpp
final_res_predictions %>% roc_auc(event, .pred_0)

# plot roc curve
# roc curve
roc_curve_xgb <- final_res_predictions %>%
